...
public class TicketServiceImpl implements ITicketService 
{
    ...
    public int applyTicket(Long companyId, Long ticketTypeId){
        System.out.println("[DEBUG] received apply, companyId is: " + companyId + "; typeId is: " + ticketTypeId);
        if (!ticketLimitCheck(companyId)){
            throw new ServiceException("申请数量过多，请关闭一些服务单后再尝试");
        }

        Long ticketDept = createTicketPermission(companyId, ticketTypeId);

        Ticket ticket = new Ticket();
        ticket.setDeptId(ticketDept);
        ticket.setCompanyId(companyId);
        ticket.setTicketTitle("Ticket");
        ticket.setTicketType(ticketTypeId);
        ticket.setStatus(1);
        ticket.setTicketType( null == ticketTypeId ? 0: ticketTypeId);

        ticketMapper.insertTicket(ticket);
        Long ticket_id = ticketMapper.selectLastInsertId();

        SysDept tempDept = deptMapper.selectDeptById(ticketDept);
        tempDept.setDeptName("Ticket " + ticket_id.toString());

        return deptMapper.updateDept(tempDept);
    }

    private boolean ticketLimitCheck(Long companyId){
        // 获取公司信息
